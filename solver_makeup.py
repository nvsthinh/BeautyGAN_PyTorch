...
class Solver_makeupGAN(object):
    def __init__(self, data_loaders, config, dataset_config):
        ...
        # Path
        self.log_path = config.log_path + config.task_name
    ...
    def log_terminal(self):
        ...
        log += '\n'
        print(log)
        save_path = os.path.join(self.log_path, 'Loss_in_Epoch_{}.txt'.format(self.e + 1))
    ...
    def train(self):
        ...
        # Start training
        self.start_time = time.time()
        for self.e in tqdm(range(start, self.num_epochs + start)):
            print('\n' + '*'*150)
            print(f'Epoch {self.e + 1}')
            print('*'*150)
            for self.i, (img_A, img_B, mask_A, mask_B) in enumerate(tqdm(self.data_loader_train)):
                ...
                self.loss['D-A-loss'] = d_loss.item()
                ...
                self.loss['D-B-loss'] = d_loss.item()
                d_loss.backward(retain_graph=True)
                ...
                if (self.i + 1) % self.ndis == 0:
                    ...
                    self.loss['G-B-loss-adv'] = g_B_loss_adv.item()
                    self.loss['G-loss-org'] = g_loss_rec_A.item()
                    self.loss['G-loss-ref'] = g_loss_rec_B.item()
                    self.loss['vgg-A'] = g_loss_A_vgg.item()
                    self.loss['vgg-B'] = g_loss_B_vgg.item()
                    if self.eye:
                        self.loss['makeup-hist-eyes'] = (g_A_eye_left_loss_his + g_A_eye_right_loss_his).item()
                    if self.lips:
                        self.loss['mkup-hist-lips'] = (g_A_lip_loss_his + g_B_lip_loss_his).item()
                    if self.skin:
                        self.loss['mkup-hist-skin'] = (g_A_skin_loss_his + g_B_skin_loss_his).item()
    ...
    def vis_train(self, img_train_list):
        ...
        save_path = os.path.join(result_path_train, '{}_{}_fake.jpg'.format(self.e + 1, self.i + 1))
